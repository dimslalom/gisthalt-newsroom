...
import { extractItem, gateClaim, approve, reject, holdForSecondSource, releaseHeldIfCorroborated, enqueuePost, reservePost, finishPost, seedAccounts, editCaption, type Ctx } from '@newsroom/pipeline';
...
describe('verification integration',()=>{
  ...
  it('rejects a missing fixture certification instead of selecting untested layouts',()=>{expect(isLayoutShippable('unknown','framed',null)).toBe(false);});
});
